...
from Knight_PIRANHAGUN import *
from Archer_PIRANHAGUN import *
from Wizard_PIRANHAGUN import *
...
class World(object):
    ...
    def generate_pathfinding_graphs(self, filename):
        ...
        while line != "":
            ...
            # Create the connections
            # len(data)-1 to prevent adding 1 error. list length error. 
            for i in range(0, len(data)-1):
                node0 = int(data[i])
                node1 = int(data[i + 1])
                distance = (Vector2(self.graph.nodes[node0].position) - Vector2(self.graph.nodes[node1].position)).length()
                # add connection back and forth
